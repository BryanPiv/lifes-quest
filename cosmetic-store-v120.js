(function(){
 const ITEMS=[
  ...
  {id:"round-glasses",name:"Trail Glasses",slot:"head",icon:"👓",price:18,rarity:"Common",description:"Simple traveling glasses for a focused companion."},
  {id:"leaf-pin",name:"Meadow Pin",slot:"charm",icon:"🍃",price:18,rarity:"Common",description:"A small leaf collected near the First Ascent."},
  {id:"emerald-band",name:"Emerald Headband",slot:"head",icon:"🥬",price:32,rarity:"Uncommon",description:"A green band that celebrates steady progress."},
  {id:"forest-trail",name:"Forest Trail",slot:"trail",icon:"🌿",price:38,rarity:"Uncommon",description:"Leaves a growing trail of green behind each step."},
  {id:"ocean-cape",name:"Ocean Cape",slot:"back",icon:"🧿",price:58,rarity:"Rare",description:"A deep-blue cape inspired by Tidehaven currents."},
  {id:"blue-orbit",name:"Azure Orbit",slot:"aura",icon:"🔵",price:62,rarity:"Rare",description:"A brilliant blue aura circling the companion."},
  {id:"violet-wings",name:"Violet Wings",slot:"back",icon:"🪽",price:92,rarity:"Epic",description:"Mystical purple wings from the edge of Eclipse Realm."},
  {id:"nebula-crown",name:"Nebula Crown",slot:"head",icon:"🔮",price:95,rarity:"Epic",description:"A crown surrounded by shifting violet starlight."},
  {id:"sun-halo",name:"Solar Halo",slot:"aura",icon:"☀️",price:145,rarity:"Legendary",description:"A golden halo that shines like the Freedom Summit."},
  {id:"dragon-wings",name:"Golden Dragon Wings",slot:"back",icon:"🐉",price:160,rarity:"Legendary",description:"Ancient golden wings reserved for legendary explorers."},
...
`;document.head.appendChild(s)}
 ...
 function showReveal(item,crate){
  const reveal=document.getElementById("lqCrateReveal");reveal.style.setProperty("--crate",crate.color);
  reveal.innerHTML='<div class="lqRevealBurst"><small>'+crate.rarity.toUpperCase()+' COSMETIC UNLOCKED</small><div class="lqRevealIcon">'+item.icon+'</div><h2>'+item.name+'</h2><p>'+item.description+'</p><div class="lqRevealActions"><button data-keep>KEEP IN INVENTORY</button><button class="equip" data-equip-reveal>EQUIP NOW</button></div></div>';
  reveal.classList.add("on");reveal.querySelector("[data-keep]").onclick=()=>reveal.classList.remove("on");reveal.querySelector("[data-equip-reveal]").onclick=()=>{reveal.classList.remove("on");activate(item.id)}
 }
 function openCrate(id){
  const crate=CRATES.find(x=>x.id===id),s=syncEarned();if(!crate)return;
  const pool=ITEMS.filter(i=>!i.achievement&&i.rarity===crate.rarity&&!s.owned.includes(i.id));
  if(!pool.length){toast(crate.rarity+" collection complete");return}
  if(s.tokens<crate.cost){toast("You need "+(crate.cost-s.tokens)+" more Journey Tokens");return}
  s.tokens-=crate.cost;const item=pool[randomIndex(pool.length)];s.owned.push(item.id);s.grants["crate:"+Date.now()+":"+item.id]=crate.id;write(s);render();showReveal(item,crate)
 }
